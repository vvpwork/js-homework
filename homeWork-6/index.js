...
class Hamburger {
    ...
    removeTopping(topping) {
         this._toppings = this._toppings.reduce((acc, val) => topping != val ?  acc.concat(val) : acc, []) ;
    }
     ...
     */
    calculatePrice() {
        return Hamburger.SIZES[this._size].price + Hamburger.STUFFINGS[this._stuffing].price + this._toppings.reduce((acc, val) => acc += Hamburger.TOPPINGS[val].price ,0)
    }
    get price(){
        return Hamburger.SIZES[this._size].price + Hamburger.STUFFINGS[this._stuffing].price + this._toppings.reduce((acc, val) => acc += Hamburger.TOPPINGS[val].price ,0)
    }
     ...
     */
    calculateCalories() {
        return Hamburger.SIZES[this._size].calories + Hamburger.STUFFINGS[this._stuffing].calories + this._toppings.reduce((acc, val) => acc += Hamburger.TOPPINGS[val].calories ,0)
    }
    get calories(){
        return Hamburger.SIZES[this._size].calories + Hamburger.STUFFINGS[this._stuffing].calories + this._toppings.reduce((acc, val) => acc += Hamburger.TOPPINGS[val].calories ,0)
    }
